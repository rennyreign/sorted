...
An agent harness submits one server-authenticated package. The ingestion service derives the tenant from its credential and allowed client mapping; it does not trust an arbitrary browser tenant field.
        ...
        "ads": [
          {
            ...
            "creative_key": "/creatives/immutable-sha256-key.webp",
...
- IDs and slugs use lowercase letters, numbers and hyphens; ad display IDs may also use uppercase letters and digits.
...
- The server computes the approval fingerprint from all client-visible execution fields and the creative content hash.

Reject the complete package on validation failure. Return structured field errors to the harness; never publish a partially accepted campaign to the client index.
