...
const AUTH_KEY = "feroza_auth"
const AUTH_EXPIRY_DAYS = 30
const QUOTE_AMOUNT = 200
const CLIENT_SLUG = "feroza"
...
export default function FerozaQuote() {
  ...
  useEffect(() => {
    ...
    if (stored) {
      try {
        const { expires, signature, accepted: storedAccepted } = JSON.parse(stored)
  ...
  const saveAuth = (data?: { signerName?: string; signedAt?: string; accepted?: boolean }) => {
    const expires = new Date().getTime() + (AUTH_EXPIRY_DAYS * 24 * 60 * 60 * 1000)
    const stored: { expires: number; signature?: { signerName: string; signedAt: string }; accepted?: boolean } = { expires }
  ...
  const handleSignatureSubmit = (e: React.FormEvent) => {
    ...
    if (signerName.trim()) {
      ...
      setIsSigned(true)
      setSignedAt(now)
      saveAuth({ signerName: signerName.trim(), signedAt: now, accepted: true })
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    if (password.toLowerCase() === "feroza2026") {
      setIsAuthenticated(true)
      setError(false)
      saveAuth()
    } else {
  ...
  const handleAcceptQuote = () => {
    setAccepted(true)
    saveAuth({ accepted: true })
    setShowAgreement(true)
  }

  const depositAmount = Math.round(QUOTE_AMOUNT / 2)
  const balanceAmount = QUOTE_AMOUNT - depositAmount
    ...
    return (
      <main className="min-h-screen flex items-center justify-center px-6">
        <div className="max-w-sm w-full">
          ...
          <h1 className="font-sans font-bold text-[#0A0A0A] text-xl mb-2">Private — For Feroza</h1>
  ...
  // Automatic progress: accepted quote/signature -> step 3. Manual override via
  // client_progress table (operator Clients tab) advances steps 3-5.
  const quoteAgreed = isSigned || accepted
  ...
  const steps = [
    {
      ...
      body: "The site has been designed and built — it\u2019s already live for you to review. You\u2019ve seen it, and you like it.",
    },
    {
      num: "02",
      title: "We agree on a quote",
      body: "Review the quote below. Accept it to move things forward — or let me know if you have any questions first.",
    },
    {
      num: "03",
      title: "We take a 50% deposit",
      body: `A deposit of £${depositAmount} secures the launch work. On receipt, the remaining details get polished and the site is prepared for launch.`,
    },
    {
      num: "04",
      title: "We polish and prepare for launch",
      body: "Final refinements — your booking link connected, your domain set up, and a full QA pass so everything is production-ready.",
    },
    {
      num: "05",
      title: "We launch your site",
      body: "Your site goes live on your own domain, bundled with Sorted Updates (edit your own content) and Sorted Tracking (see your visitors and enquiries).",
    },
  ].map((step, i) => ({
    ...step,
    ...
    <>
      <main className="max-w-[680px] mx-auto px-6 sm:px-10 pt-24 pb-32">
        ...
        <div className="mb-16">
          ...
          <p className="font-mono text-xs text-[#A3A3A3] uppercase tracking-[0.15em]">Private — for Team Feroza</p>
        </div>
        ...
        <p className="font-mono text-xs uppercase tracking-[0.15em] text-[#A3A3A3] mb-10">Team Feroza — Website Build</p>
            ...
            {[
              {
                ...
                body: "A fully designed and built website for Team Feroza — the site you\u2019ve already seen: Home, About, How I Help, Services, and a booking page. Optimised for mobile and desktop, built to introduce you, your story, and the two ways to work with you — partnering, or household services.",
              },
              {
                num: "02",
                title: "Sorted Updates",
                body: "A content management system at your site URL /cms. Every piece of text and every image is editable without touching code. Update your story, swap photos, tweak your copy — all through a simple editor. Changes go live after you hit publish. No developer needed, no waiting.",
              },
              {
                num: "03",
                title: "Sorted Tracking",
                body: "Analytics and conversion tracking set up from day one. See how many people visit your site, where they come from, and which pages lead to enquiries. The data to know what\u2019s working, what isn\u2019t, and where to focus. No guessing — just clear numbers.",
              },
            ].map((item) => (
            ...
            <div className="space-y-3 text-sm">
              <div className="flex gap-4 items-center">
                ...
                {depositPaid ? (
                  ...
                  <span className="text-[#A3A3A3] text-xs">due now to prepare for launch</span>
                )}
              </div>
              <div className="flex gap-4 items-center">
              ...
              <div className="border-t border-black/[0.06] pt-3 flex gap-4 items-center">
                ...
                <span className="text-[#0A0A0A] font-bold font-mono text-base">£{QUOTE_AMOUNT}</span>
                {depositPaid && (
                  <span className="text-[#525252] text-xs">£{QUOTE_AMOUNT - (progress?.deposit_amount ?? 0)} remaining</span>
                )}
              </div>
            </div>
          </div>

          {/* Account Details — only show once the quote is accepted */}
          {quoteAgreed && (
            <div className="p-6 bg-black/[0.02] rounded-xl border border-black/[0.06]">
              ...
              <p className="text-xs text-[#A3A3A3] mt-5 leading-relaxed">
                Please use &ldquo;Team Feroza&rdquo; as the payment reference. An invoice will be issued on receipt.
              </p>
            </div>
          )}
        </div>
            ...
            <div>
              ...
              <ul className="space-y-2 text-[#737373] text-base leading-relaxed">
                {[
                  ...
                  "Update your story, services, and booking details",
          ...
          {isSigned ? (
            <div className="bg-green-50 border border-green-200 rounded-xl p-6">
              ...
              <p className="text-green-800 text-sm mt-3 leading-relaxed">
                {depositPaid
                  ? <>Deposit received — £{progress?.deposit_amount} paid. Remaining balance of £{QUOTE_AMOUNT - (progress?.deposit_amount ?? 0)} is due on launch.</>
            ...
            >
              ...
              <div className="px-6 py-5 overflow-y-auto flex-1">
                <div className="bg-black/[0.02] rounded-xl p-5 mb-6 space-y-4 text-sm text-[#525252] max-h-64 overflow-y-auto">
                  ...
                  Sorted agrees to deliver a custom website for Team Feroza, bundled with Sorted Updates (content management system) and Sorted Tracking (analytics and conversion tracking). The website is the build Feroza has already reviewed and approved.</p>

                  <p><strong className="text-[#0A0A0A]">2. Fee</strong><br/>
                  £{QUOTE_AMOUNT} total. A 50% deposit (£{depositAmount}) is due to prepare for launch. The remaining 50% (£{balanceAmount}) is due on launch.</p>

                  <p><strong className="text-[#0A0A0A]">3. Timeline</strong><br/>
                  Once the deposit is received, the site will be polished and prepared for launch — booking link connected, domain set up, and a full QA pass. Launch follows once the balance is settled.</p>
                  ...
                  All payments to be made to ADX ENGINE LTD, NatWest, Sort Code 52-30-02, Account Number 30189489. Please use &ldquo;Team Feroza&rdquo; as the payment reference.</p>
