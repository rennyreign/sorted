    ...
    <div className="ads-content">
      ...
      <div className="ads-campaign-layout">
        <div className="ads-campaign-main">
          <div className="ads-angle-list">
            {visibleAngles.map((angle) => (
              <AngleRow key={angle.id} angle={angle} expanded={expandedAngle === angle.id} onToggle={() => toggleAngle(angle.id)} />
            ))}
          </div>
        </div>
        <div className="ads-right-rail">
          ...
          {campaign.start_date && (<div className="ads-rail-card"><h3>Key date</h3><div style={{ fontSize: 15, fontWeight: 600 }}>{campaign.start_date}</div></div>)}
          {campaign.audience && (<div className="ads-rail-card"><h3>Target audience</h3><div style={{ fontSize: 15 }}>{campaign.audience}</div></div>)}
          <div className="ads-rail-card"><h3>Channels</h3><div style={{ fontSize: 15, textTransform: "capitalize" }}>{campaign.channel}</div></div>
          {campaign.goal && (<div className="ads-rail-card"><h3>Campaign goal</h3><div style={{ fontSize: 14, color: "var(--ads-text-muted)" }}>{campaign.goal}</div></div>)}
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div className={`ads-angle ${expanded ? "expanded" : ""}`}>
      <div className="ads-angle-summary" onClick={onToggle} role="button" tabIndex={0}>
        ...
        <div className="ads-angle-thumb" style={{ background: angle.shared_creative_url ? `url(${angle.shared_creative_url}) center/cover` : "linear-gradient(135deg, #E9F3EE 0%, #F2F8DD 100%)" }} />
        ...
        <div className="ads-angle-status-summary">{statusParts.join(" · ") || "3 draft"}</div>
      ...
      {expanded && (
        <div className="ads-angle-editor">
          <div className="ads-shared-creative">
            <div style={{ width: 80, height: 80, borderRadius: 10, background: angle.shared_creative_url ? `url(${angle.shared_creative_url}) center/cover` : "linear-gradient(135deg, #E9F3EE 0%, #F2F8DD 100%)", flexShrink: 0 }} />
            <div className="ads-shared-creative-info">
              <strong>Shared creative</strong>
              <small>Used by Short, Medium and Long</small>
            </div>
            <button className="ads-btn ads-btn-secondary" style={{ height: 38, padding: "0 16px", fontSize: 14 }}><ImagePlus size={16} strokeWidth={1.75} /> Change</button>
          </div>
          <div className="ads-variants">
            {angle.variants.map((variant) => (<VariantEditor key={variant.id} variant={variant} />))}
          </div>
        </div>
      )}
    </div>
...
function VariantEditor({ variant }: { variant: any }) {
  const [text, setText] = useState({ primary_text: variant.primary_text || "", headline: variant.headline || "", description: variant.description || "" })
  return (
    <div className="ads-variant">
      <div className="ads-variant-header">
        ...
        <span className={`ads-status-pill ${statusClass(variant.approval_status as AdStatus)}`} style={{ height: 24, fontSize: 12 }}><span className="dot" />{statusLabel(variant.approval_status as AdStatus)}</span>
      </div>
      ...
      <div className="ads-variant-actions">
        <button className="ads-btn ads-btn-ghost" style={{ color: "var(--ads-green)" }}><Check size={16} strokeWidth={2} /> Approve</button>
        <button className="ads-btn ads-btn-ghost"><MessageSquare size={16} strokeWidth={1.75} /> Comment</button>
      </div>
    </div>
  )
}
