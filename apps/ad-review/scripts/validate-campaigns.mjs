...
const creative = /^\/creatives\/[a-f0-9]{64}\.webp$/
...
for (const file of files) {
  ...
  for (const concept of campaign.concepts || []) {
    ...
    for (const ad of concept.ads || []) {
      ...
      try { const url = new URL(ad.destination_url); if (url.protocol !== 'https:' || url.hostname !== 'schoolofskill.co.uk') errors.push(`${ad.id} destination is not approved`) } catch { errors.push(`${ad.id} destination is invalid`) }
