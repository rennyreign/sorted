...
export function attachImageEditor(context) {
  ...
  document.querySelectorAll('.card[data-ad-id], .ad-card').forEach(card => {
    const ad = campaign.concepts.flatMap(concept => concept.ads).find(item => item.id === (card.dataset.adId || card.querySelector('.ad-meta strong')?.textContent))
    if (!ad) return
...
function openEditor(context, campaign, ad, locked, trigger) {
  ...
  dialog.innerHTML = `<header class="editor-heading"><div><p class="eyebrow">${esc(ad.id)} · ${esc(campaign.name)}</p><h2 id="image-editor-title">Choose the right image.</h2><p>Preview your choice with the ad before saving.</p></div><button type="button" data-close aria-label="Close image editor">✕</button></header>
    ...
    </section><section class="editor-preview" aria-label="Ad preview"><p class="eyebrow">Preview · ${esc(ad.ratio)}</p><div class="preview-card"><div class="adIdentity"><span class="avatar">${esc(state.data.tenant.name[0])}</span><div><strong>${esc(state.data.tenant.name)}</strong><small>Sponsored</small></div></div><p class="adText">${esc(ad.primary_text)}</p><div class="crop-preview" style="aspect-ratio:${esc(ad.ratio.replace(':','/'))}"><img id="selected-preview" alt=""></div><div class="adDestination"><div><h3>${esc(ad.headline)}</h3><p>${esc(ad.description)}</p></div><span class="mockCta">${esc(ad.cta.replaceAll('_',' '))}</span></div></div><p id="selection-name"></p><div id="crop-controls"><label>Horizontal position<input id="crop-x" type="range" min="0" max="100" value="${crop.x}"></label><label>Vertical position<input id="crop-y" type="range" min="0" max="100" value="${crop.y}"></label><button type="button" id="reset-crop">Centre image</button></div><p class="crop-note" id="crop-note"></p></section></div>
